...
# ═══════════════════════════════════════════════════════════════════════════════
# PLOT 1: Attacker Throughput & PRB Allocation during Attack and Throttle
# ═══════════════════════════════════════════════════════════════════════════════
...
line_thp, = ax1.plot(attacker['t_sec'], attacker['thp_ul_mbps'],
                     color=C_ATK_THP, linewidth=2.2, label='Laju Data UL Penyerang (Throughput)',
                     zorder=3)
ax1.set_xlabel('Waktu Eksperimen (detik)', fontweight='bold')
ax1.set_ylabel('Laju Data Uplink (Mbps)', color=C_ATK_THP, fontweight='bold')
...
line_prb, = ax1_prb.plot(attacker['t_sec'], attacker['prb_usage_ul_ratio'] * 100,
                         ...
                         label='Alokasi PRB Uplink Penyerang (%)', zorder=2)
ax1_prb.set_ylabel('Alokasi PRB Uplink (%)', color=C_ATK_PRB, fontweight='bold')
...
ax1.annotate('Laju Data Terkunci ~1,1 Mbps\n(PRB Terbatasi 5%)',
             xy=(85.0, 1.1), fontsize=10, fontweight='bold',
             color='#37474F', xytext=(90.0, 8.0),
             arrowprops=dict(arrowstyle='->', color='#37474F', lw=1.2))

ax1.set_title('Profil Laju Data (Throughput) dan Alokasi PRB UE Penyerang (RNTI 6)\nSebelum, Selama, dan Sesudah Mitigasi Aktif',
...
fig1.tight_layout()
for d in OUTPUT_DIRS:
    fig1.savefig(os.path.join(d, 'eval_attacker_throughput.png'))
print(f"Saved eval_attacker_throughput.png to all destinations")
plt.close(fig1)


# ═══════════════════════════════════════════════════════════════════════════════
# PLOT 2: Victim/Normal UE Throughput & PRB Allocation during Attacker Mitigation
# ═══════════════════════════════════════════════════════════════════════════════
...
line_vic_thp, = ax2.plot(victim['t_sec'], victim['thp_dl_mbps'],
                         color=C_VIC_THP, linewidth=2.2, label='Laju Data DL UE Normal (Throughput)',
                         zorder=3)
ax2.set_xlabel('Waktu Eksperimen (detik)', fontweight='bold')
ax2.set_ylabel('Laju Data Downlink (Mbps)', color=C_VIC_THP, fontweight='bold')
...
line_vic_prb, = ax2_prb.plot(victim['t_sec'], victim['prb_usage_dl_ratio'] * 100,
                             ...
                             label='Alokasi PRB Downlink UE Normal (%)', zorder=2)
ax2_prb.set_ylabel('Alokasi PRB Downlink (%)', color=C_VIC_PRB, fontweight='bold')
...
line_atk_ref, = ax2.plot(attacker['t_sec'], attacker['thp_ul_mbps'],
                         ...
                         label='Ref: Throughput Penyerang (Mbps)', zorder=1)
...
# Annotations
ax2.annotate('Serangan Banjir Data\n(Attacker Flood Active)',
...
ax2.annotate('Unduh UE Normal Aktif\nThroughput Puncak ~114,87 Mbps\n(QoS Terjaga Selama Mitigasi)',
             xy=(68.0, 114.87), fontsize=10, fontweight='bold',
             color=C_MIT_LINE, xytext=(72.0, 80.0),
             arrowprops=dict(arrowstyle='->', color=C_MIT_LINE, lw=1.5))

ax2.set_title('Profil Laju Data (Throughput) dan Alokasi PRB UE Normal (RNTI 5)\nSelama Fase Serangan dan Mitigasi Aktif',
              fontsize=13, fontweight='bold', pad=12)
ax2.set_xlim(*XLIM2)
...
fig2.tight_layout()
for d in OUTPUT_DIRS:
    fig2.savefig(os.path.join(d, 'eval_victim_throughput.png'))
print(f"Saved eval_victim_throughput.png to all destinations")
plt.close(fig2)


# ═══════════════════════════════════════════════════════════════════════════════
# PLOT 3: Attacker Restore Phase Throughput & PRB Allocation [NEW]
# ═══════════════════════════════════════════════════════════════════════════════
...
line_res_thp, = ax3.plot(attacker['t_sec'], attacker['thp_ul_mbps'],
                         color=C_ATK_THP, linewidth=2.2, label='Laju Data UL Penyerang (Throughput)',
                         zorder=3)
ax3.set_xlabel('Waktu Eksperimen (detik)', fontweight='bold')
ax3.set_ylabel('Laju Data Uplink (Mbps)', color=C_ATK_THP, fontweight='bold')
...
line_res_prb, = ax3_prb.plot(attacker['t_sec'], attacker['prb_usage_ul_ratio'] * 100,
                         color=C_ATK_PRB, linewidth=1.6, linestyle='--',
                         label='Alokasi PRB Uplink Penyerang (%)', zorder=2)
ax3_prb.set_ylabel('Alokasi PRB Uplink (%)', color=C_ATK_PRB, fontweight='bold')
...
# Annotations
ax3.annotate('Akses RESTORE Terdeteksi\n(Pemulihan Jaringan)',
...
ax3.annotate('Laju Data Pulih ~10,1 Mbps\n(Tanpa Batasan PRB)',
             xy=(145.0, 10.11), fontsize=10, fontweight='bold',
             color='#37474F', xytext=(148.0, 5.0),
             arrowprops=dict(arrowstyle='->', color='#37474F', lw=1.2))

ax3.set_title('Profil Pemulihan Laju Data (Throughput) UE Penyerang (RNTI 6)\nSetelah Pengiriman Perintah RESTORE (Bebas Batasan)',
              fontsize=13, fontweight='bold', pad=12)
ax3.set_xlim(*XLIM3)
...
fig3.tight_layout()
for d in OUTPUT_DIRS:
    fig3.savefig(os.path.join(d, 'eval_attacker_restore.png'))
print(f"Saved eval_attacker_restore.png to all destinations")
plt.close(fig3)

print("\nAll Indonesian plots generated successfully!")
