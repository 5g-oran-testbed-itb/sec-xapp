...
MITIGATE_CONF="/home/telmat/sec-xapp/deploy/ric/my_xapp_mitigate.conf"
...
tmux send-keys -t "$SESSION:0.1" \
    "echo '=== [Pane 1] srsGNB via SSH ===' && sleep 3 && \
     ...
     'cd $RAN_GNB_DIR && echo \"$RAN_PASS\" | sudo -S pkill -9 gnb 2>/dev/null; echo \"$RAN_PASS\" | sudo -S killall -9 gnb 2>/dev/null; sleep 1; \
      echo \"$RAN_PASS\" | sudo -S stdbuf -oL $RAN_GNB_BIN -c $RAN_GNB_CONF 2>&1 | tee /tmp/gnb.log'" Enter
