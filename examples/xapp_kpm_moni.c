...
void log_kpm_measurements(kpm_ind_msg_format_1_t const* msg_frm_1)
{
  ...
  printf("  meas_info_lst_len = %lu, meas_data_lst_len = %lu\n",
         (unsigned long)msg_frm_1->meas_info_lst_len,
         (unsigned long)msg_frm_1->meas_data_lst_len);
  fflush(stdout);
  ...
  for (size_t j = 0; j < msg_frm_1->meas_data_lst_len; j++) {
    ...
    printf("  data_item[%lu] meas_record_lst = %p\n", (unsigned long)j, (void*)data_item.meas_record_lst);
    fflush(stdout);

    for (size_t i = 0; i < msg_frm_1->meas_info_lst_len; i++) {
      const meas_info_format_1_lst_t info_item = msg_frm_1->meas_info_lst[i];

      printf("  info_item[%lu] meas_type.type = %d, label_info_lst_len = %lu\n",
             (unsigned long)i, info_item.meas_type.type,
             (unsigned long)info_item.label_info_lst_len);
      fflush(stdout);

      for (size_t z = 0; z < info_item.label_info_lst_len; z++) {
        const label_info_lst_t label_info = info_item.label_info_lst[z];
        printf("  record index = %lu, ", (unsigned long)(i + z));
        fflush(stdout);

        const meas_record_lst_t record_item = data_item.meas_record_lst[i + z];
        printf("record_item.value = %d\n", record_item.value);
        fflush(stdout);

        if (record_item.value >= END_MEAS_VALUE || get_meas_value[record_item.value] == NULL) {
          printf("  Skipping unsupported meas value type %d\n", record_item.value);
          continue;
        }
...
void sm_cb_kpm(sm_ag_if_rd_t const* rd)
{
  ...
  static int counter = 1;
  {
    ...
    printf("\n%7d KPM ind_msg hdr collectStartTime = %lu\n", counter, hdr_frm_1->collectStartTime);
    printf("%7d KPM ind_msg type = %d\n", counter, ind->msg.type);
    fflush(stdout);

    if (ind->msg.type == FORMAT_1_INDICATION_MESSAGE) {
      printf("%7d Processing FORMAT_1, num_meas = %u\n", counter, ind->msg.frm_1.meas_data_lst_len);
      fflush(stdout);
      log_kpm_measurements(&ind->msg.frm_1);
    } else if (ind->msg.type == FORMAT_3_INDICATION_MESSAGE) {
      printf("%7d Processing FORMAT_3, num_ue = %u\n", counter, ind->msg.frm_3.ue_meas_report_lst_len);
      fflush(stdout);
...
static
kpm_act_def_t fill_report_style_5(ric_report_style_item_t const* report_item)
{
  assert(report_item != NULL);
  assert(report_item->act_def_format_type == FORMAT_5_ACTION_DEFINITION);
  kpm_act_def_t act_def = {.type = FORMAT_5_ACTION_DEFINITION};

  // Fill UE ID list - use wildcard by setting len to 0
  // srsRAN will report for all connected UEs
  act_def.frm_5.ue_id_lst_len = 2;
  act_def.frm_5.ue_id_lst = ecalloc(2, sizeof(ue_id_e2sm_t));
  ...
  act_def.frm_5.ue_id_lst[0].gnb.gnb_cu_ue_f1ap_lst = ecalloc(1, sizeof(uint64_t));
  ...
  act_def.frm_5.ue_id_lst[1].gnb.gnb_cu_ue_f1ap_lst = ecalloc(1, sizeof(uint64_t));
  act_def.frm_5.ue_id_lst[1].gnb.gnb_cu_ue_f1ap_lst[0] = 2;

  // Fill Action Definition Format 1 (metric list)
...
int main(int argc, char* argv[])
{
  ...
  for (size_t i = 0; i < nodes.len; ++i) {
    ...
    for (size_t j = 0; j < sz_report_styles; j++) {
      ...
      // Skip unsupported report styles
      ric_service_report_e const rst = report_item->report_style_type;
      printf("[xApp]: Report Style %zu -> type = %d\n", j, rst);
      if (rst >= END_RIC_SERVICE_REPORT || get_kpm_act_def[rst] == NULL) {
        printf("[xApp]: Skipping unsupported report style type %d\n", rst);
        continue;
      }
