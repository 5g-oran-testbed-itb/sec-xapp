#!/usr/bin/env python3
"""
plot_learning_curves_v5.py
Plots the training and validation learning curves for:
1. LSTM-AE v5 (models/lstm_ue_v5_losses.json)
2. GRU-AE v5 (models/gru_ue_v5_losses.json)
"""
...
def plot_curve(json_path, title, output_path, model_name, train_color='#4E79A7', val_color='#F28E2B', marker_color='#E15759'):
    ...
    # Vector PDF beside the PNG — these figures go into the printed document.
    pdf_path = os.path.splitext(output_path)[0] + '.pdf'
    plt.savefig(pdf_path)
    print(f"[OK] Saved vector version to {pdf_path}")
    plt.close()

def main():
    os.makedirs('eval_figures/per_ue_v5', exist_ok=True)
    
    # 1. Plot LSTM v5 learning curve
    plot_curve(
        'models/lstm_ue_v5_losses.json',
        'LSTM-Autoencoder (v5) Learning Curve - per UE',
        'eval_figures/per_ue_v5/eval_learning_curve_lstm.png',
        'LSTM-AE v5',
    ...
    # 2. Plot GRU v5 learning curve
    plot_curve(
        'models/gru_ue_v5_losses.json',
        'GRU-Autoencoder (v5) Learning Curve - per UE',
        'eval_figures/per_ue_v5/eval_learning_curve_gru.png',
        'GRU-AE v5',
