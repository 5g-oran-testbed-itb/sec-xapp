...
static void init_onnx(void) {
    ...
    const char* model_path = "/home/telmat/xapp/security-xapp/security_model.onnx";
...
static void sm_cb_mac(sm_ag_if_rd_t const* rd, global_e2_node_id_t const* e2_node)
{
  ...
  for(uint32_t i = 0; i < msg->len_ue_stats; ++i){
    ...
    if (t < WINDOW_SIZE) {
        ...
        ue_buffers[ue_idx].features[t][10] = 0.0f;
        ue_buffers[ue_idx].features[t][11] = 0.0f;

        ue_buffers[ue_idx].count++;

        if (ue_buffers[ue_idx].count == WINDOW_SIZE) {
            run_inference(s->rnti, ue_idx);
        }
    }
  }
}
...
static kpm_act_def_t fill_report_style_5(ric_report_style_item_t const* report_item) {
  ...
  act_def.frm_5.ue_id_lst_len = 0;
  act_def.frm_5.ue_id_lst = NULL;
...
/* Automatic RC mitigation is disabled by default because srsRAN RC Bug #468
 * causes the gNB E2 agent to crash/reset after receiving an RC Control msg.
 * Enable with --mitigate flag once srsRAN is patched or on a conformant gNB. */
...
{
    static const char *path = "/tmp/xapp_label";
    ...
    char scenario[64] = "unknown";
    char attacker[8]  = "none";
    long long epoch_ms = 0;
    int new_label = 0;
    if (fscanf(f, "%d,%63[^,],%7[^,],%lld", &new_label, scenario, attacker, &epoch_ms) < 1) {
        fclose(f);
        return;
    }
    fclose(f);

    if (new_label != g_label) {
        printf("[LABEL] %lld  %s -> label %d  attacker=%s\n",
               epoch_ms, scenario, new_label, attacker);
        log_scenario_event(epoch_ms, "START", new_label, scenario, attacker);
        g_label = new_label;
    }
}
/* -------------------------------------- */
...
static int test_csv_writer(void)
{
    ...
    /* label = 0 is the last field */
    char* last_comma = strrchr(buf, ',');
    if (!last_comma || atoi(last_comma + 1) != 0) {
        fclose(f);
        printf("[FAIL] label != 0 at end of row:\n  %s\n", buf);
        return 1;
    }
...
static void sm_cb_kpm(sm_ag_if_rd_t const* rd, global_e2_node_id_t const* e2_node)
{
  ...
  if (kpm->msg.type == FORMAT_1_INDICATION_MESSAGE) {
    ...
    if (msg_frm_1->meas_data_lst_len == 0) {
      if (!is_cu_cp) {
        ...
        if (empty_now_ms - g_last_csv_ms >= 90) {
          rule_based_detect(&g_cell, (long long)empty_now_ms);
    ...
    if (!is_cu_cp) {
      ...
      if (kpm_now_ms - g_last_csv_ms >= 90) {
        csv_trainer_write(&g_csv, &g_cell);
      ...
      if (sev == 2 && !g_throttle_active) {
        g_pending_throttle = 1; /* request throttle */
      } else if (sev == 0 && g_throttle_active) {
...
static int test_two_stage_detection(void)
{
    ...
    /* Test C: 25s saturation does NOT reach Stage 2 CRITICAL */
    ids_reset();
    ids_init(100);
    m.prb_used_ul = 85.0f;
    ts = 10000LL;
